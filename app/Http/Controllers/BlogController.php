...
class BlogController extends Controller
{
    ...
    public function insert_blog(Request $request)
    {
       ...
       $save->is_publish=trim($request->is_puplish);
    ...
    BlogTagsModel::InsertDeleteTag($save->id,$request->tags);
       return redirect('panel/blog/list')->with('success',"Blog succesfuly created");
    ...
    if(!empty($request->file('image_file')))
    {
        ...
        {
            /*                   هذا شرط في حالة غيرنا الصورة يمسح الصورة الاولى */
    ...
    $save->save();
    BlogTagsModel::InsertDeleteTag($save->id,$request->tags);
       return redirect('panel/blog/list')->with('success',"Blog succesfuly Updated");
    ...
    public function delete_blog($id)
{
    ...
    $save->is_delete=1;
    $save->save();
    return redirect()->back()->with('success',"Blog successfuly deleted");
}
}
